<?php
	...
	switch ($_GET["op"]) {
			...
			$numero = $parte_izquierda."".$suma;	
				...
				if(empty($_POST["txtIdVenta"])){
					...
					if($objVenta->Modificar($idventa,$idpedido, $idusuario,$tipo_venta,$tipo_comprobante,$serie_comprobante,$num_comprobante,$impuesto,$total,$estado)){
						echo "La información de la Venta ha sido actualizada.";
					}else{
						echo "La información de la Venta no ha podido ser actualizada.";
					}
				}
        ...
        case "EnviarCorreo":
			...
			//$mail->isHTML(true);
			$mail->Host = "$server";
			$mail->From = "info@perurunners.com"; //correo institucional
			$mail->FromName = "$sucursal - Administracion";
			$mail->Subject = "$sucursal - Detalle de compra";
			$mail->addAddress("$result");
			$mail->addAttachment("$server/intranet/Reportes/reportes",'Reporte-Venta'.$idPedido.'.pdf');
			$mail->MsgHTML("Puede ver el detalle de su compra haciendo click <a href='".$server."/intranet/Reportes/exVenta.php?id=".$idPedido."'> Aqui</a>");
			//$mail->AltBody = 'This is plain-text message body';
			//$mail->body="Puede ver el detalle de su compra haciendo click <a href='$server/intranet/Reportes/exVenta.php?id=".$idPedido."'> Aqui</a>";
			if ($mail->Send()) {
