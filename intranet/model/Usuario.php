<?php
	...
	where e.login = '$user' and e.clave = '$pass' and u.estado <> 'C'";
		...
		public function Ingresar_Socio($user, $pass){
			global $conexion;
			$sql = "SELECT * FROM socio so INNER JOIN pais pa ON so.id_pais=pa.id_pais INNER JOIN distritos di ON so.id_dist=di.id_distrito INNER JOIN sedes se ON so.id_sede=se.id_sede INNER JOIN bancos ba ON so.id_banco=ba.id_banco where login = '$user' and clave = '$pass'";
			$query = $conexion->query($sql);
			return $query;
		}
