...
$imagen_no_disponible="tumb/imagen_no_disponible.gif";
$insert=mysqli_query($con,"insert into productos (tumb, img, status_producto) values ('$imagen_no_disponible','$imagen_no_disponible','0')");
$sql_last=mysqli_query($con,"select LAST_INSERT_ID(id_producto) as last from productos order by id_producto desc limit 0,1");
					...
					while ($rw_cat=mysqli_fetch_array($query_cat)){
						...
						if ($nums2>0){
							...
							while ($rw2_cat=mysqli_fetch_array($sql2_cat)){
								...
								?>
								<option value="<?php echo $id2_categoria;?>" ><?php echo "&nbsp;&nbsp;$nombre_categoria > ".$nombre2_categoria;?></option>
								<?php
							}
							echo "</optgroup>";
						} else {
